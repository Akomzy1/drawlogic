...
- **Tokens.** Fonts: Inter and JetBrains Mono; the prototype's `tokens.css` defines Source Sans 3, IBM Plex Mono and Material Symbols Outlined. Colours not in the tokens: `rgba(255,255,255,0.92)` (`.concept-tag`), `rgba(210,232,255,0.33 / 0.20 / 0.13)` (`.stamp`, `.stamp-top`).
...
Note: the export places the Teach me first, site-feasibility and preview-video sections on this page rather than on Idea mode and Render studio (DESIGN_GAPS rows 5 and 6). Build them here, as exported, until a re-export moves them.
35 failing tests.
...
Interactive: NGN / USD toggle in "Naira pricing via Paystack". In the export it is inert (USD leaves NGN selected); the tests hold the site to the export until DESIGN_GAPS row 20 is decided.
