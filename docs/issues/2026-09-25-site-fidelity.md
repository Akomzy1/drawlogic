...
**Tests:** `site/tests/` (TESTING.md layer 5a) on `main` (PRs #1 and #2), following the 25 Sept re-export. Do not edit them; if one is wrong, reply here citing the prototype or the PRD clause.
...
**Tested:** a static build of the `ddl-engine` worktree's `site/` as of 25 Sept (source last changed 20 Sept), built in a scratch copy; nothing in the worktree was modified. Morning run, before the re-export and the Decision 12 and font checks: 540 tests at 375/768/1280, 10 passed, 128 skipped (single-width checks), 402 failed; the per-page findings below are unchanged by the re-export except where noted. Every failure below is a real difference from the prototype or the PRD; examiner-side problems found during the run were fixed in the tests first (end of this file).

**Work in this order.** Home first, then the eleven other pages in the prototype's navigation order. The 25 Sept re-export (DESIGN_GAPS rows 5 and 6) moved Teach me first and site feasibility to Idea mode and the preview video to Render studio; the tests and the page lists below follow it. For each page, build the listed sections in order, with the prototype's copy (from `site/content/*.json`), tokens, assets from the manifest and the interactive behaviour listed. The tests record what each prototype control does and hold the site to the same outcome, except where a decided rule overrides the export (the Lagos toggle, Decision 12).
...
### 2. Idea mode — `/idea-mode` — page missing

Sections, in order: Describe it. See it. · Two or three options, the numbers, and a render · Drop a pin. Draw the boundary. See what fits. · Everything it filled in for you, in plain words · Want to understand why, before you see it? · A concept leaves here labelled as a concept · When you are ready, one click hands it over · Free to try · Plainly answered · Start with a sentence.
Interactive: site-feasibility options A · Courtyard / B · Linear / C · Two blocks (each changes the plot figures) and the boundary polygon (vector, with "traced" and the imagery date as text) in "Drop a pin…"; Teach me first switch (starts on; switching changes the panel) in "Want to understand why…"; currency toggle (GBP, USD, NGN) in "Free to try".

### 3. For professionals — `/for-professionals` — page missing
...
### 4. Render studio — `/render-studio` — page missing

Sections, in order: Rendered from the drawing, not from a screenshot. · Drawing and render, side by side, same hash · A camera move on the drawing you already have. · What you put in · Four variants, one fixed camera · Every render carries its drawing · Renders are metered · Render from a drawing you already have.
Interactive: preview-video tile in "A camera move…" (never autoplays; the play control starts it; "Generated preview — not a model render"; no fidelity score; Stage 3 Studio note); currency toggle (GBP, USD, NGN) in "Renders are metered".

### 5. Profile marketplace — `/profile-marketplace` — page missing
...
### 6. Find a signer — `/find-a-signer` — page missing
...
### 7. Developers — `/developers` — page missing
...
### 8. Students & educators — `/students-and-educators` — page missing

Sections, in order: It teaches before it draws. · a one-line pointer ("The same critique is available to anyone in Idea mode — switch on Teach me first. Open Idea mode") · Six steps, in this order · A rough sketch, the principles, and what it found · Worked answers unlock after you try · Every submission says how much was yours · Set the exercise. Read the record. · Plainly answered · Learn the reasons first.

### 9. Regulators & manufacturers — `/regulators-and-manufacturers` — page missing
