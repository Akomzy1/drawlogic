...
6. **Eval harness** (`fixtures/eval/`) — 200-drawing set (mixed PDF quality, vector and scanned, UK/US/NG conventions) with ground-truth DDL; metrics: object recall/precision, dimension accuracy, confidence calibration (ECE), interpretation-card acceptance. Runs nightly; provider comparison report.
...
## Fixture policy
Fixtures are the spec. Engine never edits them. A wrong fixture is fixed by Claude Code in a PR that explains why, and the change is reviewed against the PRD.
