...
| `contracts/providers/render.ts` | Render interface: engine `diffusion | generative_video | raytraced | voice`; inputs (line-art, depth, material map, still, end frame, or for voice: script + camera path); outputs (asset, drawing_hash, fidelity score or null, label key) | 7.6, 5A.1a, FR-132-135 |
...
| `contracts/copy.json` | Stamp closing sentence, preview label, concept watermark text, student watermark text, "checks not performed — none" text, **banned words list** (with scopes: everywhere; outside Studio output; estimate output — quote, guaranteed price, fixed price, PRD §5B), estimate labels ("Indicative — not a quote", "This is an estimate, not a quotation.", "unverified rates", "Workspace rates — not independently verified", "No verified cost data for [jurisdiction] — no cost shown") | §8 NFR, trust rules 7–8 |
