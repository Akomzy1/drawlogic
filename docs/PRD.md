...
11. **Free access is never conditional on posting, sharing or following.** Reach comes from branding, optional sharing with disclosed rewards, and portfolios (§7.12, FR-165–172).
...
- FR-160 (P1, Stage 2) **Material references.** Any material in the DDL may carry a colour reference (RAL, NCS, Pantone, BS 4800, or a manufacturer paint code) and/or a user-uploaded sample photo. Both are stored against the `material_id` with `source: user` and appear in annotations, schedules and the finishes list exactly as supplied.
- FR-161 (P1, Stage 2) **Render conditioning.** The material map passed to the render provider carries each material's colour value (converted from its code via a colour library) and, where supplied, the sample photo as a texture reference limited to that material's region mask. No reference → the material is rendered from its name and profile default only.
- FR-162 (P1, Stage 2; PBR textures Stage 3) **Manufacturer material data.** Manufacturer product profiles (FR-66) carry colour values and product images at Stage 2, and PBR texture sets at Stage 3 for Studio renders (FR-123). A render may use manufacturer data only when a signed product profile supplies it.
- FR-163 (P1, Stage 2) **Material basis label.** Every still and preview clip states, for each visible material, its basis: "manufacturer texture", "sample photo", "colour code [code]" or "description only". Shown per material on hover and summarised in the render strip, e.g. "Material basis: 2 colour codes · 1 sample photo · 1 description only".
- FR-164 (P1, Stage 2) **Colour check.** For materials with a colour code, compare the rendered region's mean colour against the target (ΔE2000); above the threshold in `contracts/render.thresholds.json` the material is labelled "colour approximate". This flags, it does not fail the render. Initial threshold set from the eval harness.
- In render and preview outputs, "exact match", "exact colour" and "true colour" are banned words.
...
**Student reach without compulsion (v0.2.12; Stage 2 unless marked).** Principle: free student access is never conditional on posting, sharing, following or tagging anything. Reach comes from branding, optional sharing with disclosed rewards, and portfolios.
- FR-165 (P0, Stage 1) **Export branding.** Every student export carries, alongside the existing "Student — educational use" watermark, a small "Made with Drawlogic" mark. Not removable on the student plan.
- FR-166 (P1) **Share card.** After a Learn exercise, offer an optional one-tap share card: an auto-generated before/after (sketch → critique → drawing) sized for Instagram, TikTok and LinkedIn, carrying the "Made with Drawlogic" mark and the "Illustrative" caption where any generated imagery appears.
- FR-167 (P1) **Post-assessment unlock.** The share card and any public posting of an exercise are available only after the student marks that exercise as assessed (or submitted, where the institution allows). Before that, the share action is shown disabled with "Available after your work is marked."
- FR-168 (P1) **Disclosed rewards.** Sharing may earn extra render or Idea credits, never basic free access. When a share earns a reward, the card and suggested caption include the disclosure label required for the platform and jurisdiction ("#ad" or "Gifted: Drawlogic credits"), added automatically and not removable in the share flow.
- FR-169 (P1) **Portfolio pages.** Opt-in public profile on drawlogic.com showing selected exercises with their Learn story and integrity summary. Private by default; any item removable at any time; the whole page deletable.
- FR-170 (P1) **Referrals and ambassadors.** Credits for each classmate who verifies and completes an exercise. Campus ambassador programme paid in credits or cash, with ambassadors required to disclose the relationship in every post.
- FR-171 (P2) **Design challenges.** Monthly briefs with prizes and a public gallery of entries submitted by choice. Run under the applicable prize promotion rules (UK CAP Code section 8 and equivalents) and each platform's contest terms; entry never requires posting on a personal feed.
- FR-172 (P0) **Minors.** For any account indicating the user is under 18, share cards, portfolio pages, referrals, ambassador roles and challenge entry are disabled.
...
### 7.14 Precedents: inspiration that points to the original (v0.2.13; Stage 2 unless marked)

**Principle.** Precedents are for looking at, with credit and a link to where the work is published. Generation takes qualities, never copies of a specific protected design. Nothing from someone else's design enters a drawing without being marked as such.

- FR-173 (P1) **Precedent search.** In Idea and Learn mode, a search returns precedent cards: project name, designer, location, year, a short description written in Drawlogic's own words, the qualities it illustrates (e.g. "courtyard plan, deep eaves, cross-ventilation"), and a link to the original publication. Runs through the LLM provider with a web search tool (new task `precedent_search` in `contracts/models.json`, default Sonnet 5). Results are filtered to the project's building type and climate where known.
- FR-174 (P1) **No hosted images without a licence.** Precedent cards show no third-party photographs or drawings unless a licence record exists for that asset (publisher partnership, designer opt-in, manufacturer, or public domain). Otherwise the card is text plus link.
- FR-175 (P1) **Qualities, not copies.** The user can pin qualities from a precedent card into an Idea or Draft brief; generation uses those qualities as constraints. A prompt asking to replicate a specific named, protected building or a named designer's specific work is declined with a short explanation, and the qualities of that work are offered instead. Works in the public domain (e.g. historic buildings whose copyright has expired) are exempt and labelled as such.
- FR-176 (P1) **Provenance for precedent influence.** Any constraint pinned from a precedent is recorded with `source: reference` and the precedent's attribution, and appears in the assumptions panel or interpretation card ("Deep eaves — from precedent: [project], [designer]").
- FR-177 (P0, Stage 1) **User-uploaded references.** When a user uploads a photo or drawing of someone else's work as a reference, they confirm they have the right to use it for this purpose. Elements derived from it are `source: reference` (as now) with the upload's stated origin.
- FR-178 (P2, Stage 3) **Designer-contributed precedents.** Practices and designers publish their own projects in the marketplace under a licence grant: images, credit, qualities, and a "hire this designer" link. They can withdraw them at any time.
- FR-179 (P2, Stage 3) **Manufacturer case studies.** Manufacturers with product listings may supply built projects using their products, with the project owner's permission, shown as precedent cards linked to the product profile.
- FR-180 (P1) **Learn mode precedents** come from cited texts already in use, public-domain works, and opt-in content only.

**Copy rules.** Precedent descriptions are paraphrased in Drawlogic's own words; no copying of publisher text beyond a short quotation with attribution. **Legal review:** Decision 19 (DECISIONS.md), OPEN, before Stage 2 launch.
...
| Precedent search (`precedent_search`, FR-173; Stage 2) | Claude Sonnet 5 with a web search tool | set in `models.json` | Cards in Drawlogic's own words with a link to the original |
