...
## 13c. Precedents point to the original
Precedents are for looking at, with credit and a link to where the work is published (PRD §7.14, FR-173–180). Generation takes qualities, never copies of a specific protected design; nothing from someone else's design enters a drawing unmarked. Descriptions are paraphrased in Drawlogic's own words — no publisher text beyond a short quotation with attribution. **Tests:** no precedent card renders a third-party image unless the card carries a licence record (publisher partnership, designer opt-in, manufacturer, or public domain); a prompt to replicate a named, protected building or a named designer's specific work takes the decline-and-offer-qualities path, while a public-domain work passes and is labelled as such; every quality pinned from a precedent carries `source: reference` and the precedent's attribution into the assumptions panel or interpretation card; an uploaded reference of someone else's work cannot be used until the user confirms their right to use it (FR-177, Stage 1).
