...
| `prd` (1280) | US-dollar amounts, seats and comparison-table cells agree with PRD §10 (`fixtures/prd-governed.json`). Every currency toggle (including Lagos) switches every displayed price to the chosen currency, with the same number of prices in each; NGN amounts are Decision 12's, USD amounts PRD §10's. Values tied to an OPEN decision are annotated, not asserted |
