...
| `prd` (1280) | US-dollar amounts, seats and comparison-table cells agree with PRD §10 (`fixtures/prd-governed.json`). Every currency toggle (including Lagos) switches every displayed price to the chosen currency, with the same number of prices in each; NGN amounts must be in PRD §10's Nigeria tier prices table, USD amounts its plan prices — both read from `docs/PRD.md` at run time, so the PRD is the single source. Values tied to an OPEN decision are annotated, not asserted |
...
| `tokens` | Every rendered colour is one the prototype's `tokens.css` defines or the prototype renders. Fonts: every family named anywhere in a rendered font stack, and every web font the page declares, is one the prototype's stacks or `tokens.css` name (Source Sans 3, IBM Plex Mono, Material Symbols Outlined and their fallbacks); any other family fails |
...
| `interactive` | Hero wall, site-feasibility options and boundary polygon, preview-video tile, currency toggle, Teach me first toggle, tabs and the signing gate behave as the prototype's do, in full and reduced motion. The same checks run against the prototype when the fixture is generated, and must pass there. Where a DECIDED rule replaces the export's behaviour (`behaviour_overrides` in `fixtures/prd-governed.json`, e.g. the Lagos toggle under PRD §10 / Decision 12), `prd` checks it instead |
